(function(){const a=document.createElement("link").relList;if(a&&a.supports&&a.supports("modulepreload"))return;for(const t of document.querySelectorAll('link[rel="modulepreload"]'))n(t);new MutationObserver(t=>{for(const s of t)if(s.type==="childList")for(const i of s.addedNodes)i.tagName==="LINK"&&i.rel==="modulepreload"&&n(i)}).observe(document,{childList:!0,subtree:!0});function r(t){const s={};return t.integrity&&(s.integrity=t.integrity),t.referrerPolicy&&(s.referrerPolicy=t.referrerPolicy),t.crossOrigin==="use-credentials"?s.credentials="include":t.crossOrigin==="anonymous"?s.credentials="omit":s.credentials="same-origin",s}function n(t){if(t.ep)return;t.ep=!0;const s=r(t);fetch(t.href,s)}})();const g="data/md_impact_analysis.json",h="data/counties.geojson",u=new Intl.NumberFormat("en-US",{style:"currency",currency:"USD",maximumFractionDigits:0}),b=o=>o?new Date(o).toLocaleDateString("en-US",{month:"short",day:"numeric",year:"numeric"}):"N/A";async function w(){try{const[o,a]=await Promise.all([fetch(g),fetch(h)]);if(!o.ok)throw new Error(`Impact data failed: ${o.statusText}`);if(!a.ok)throw new Error(`Geo data failed: ${a.statusText}`);const r=await o.json(),n=await a.json();_(r.meta.generated_at),x(r);const t=n.features||n;$(t,r.visualization_data.breakdown_by_county),v(r.visualization_data.breakdown_by_county),T(r.raw_impact_events.tier1)}catch(o){console.error("Dashboard Crash Details:",o),document.querySelector("main").innerHTML=`
    ...
    `}}function _(o){const a=new Date(o);document.getElementById("last-updated").textContent=`Last Updated: ${a.toLocaleString()}`}function x(o){const a=o.summary_stats,r=o.visualization_data,n=document.getElementById("topline-container"),t=(p,y,m,e)=>`
    <div class="stat-card ${e}">
      <div class="stat-label">${p}</div>
      <div class="stat-value text-${e}">${y}</div>
      <div class="stat-sub">${m}</div>
    </div>
  `,s=new Set;o.raw_impact_events&&o.raw_impact_events.tier1&&o.raw_impact_events.tier1.forEach(p=>{p.recipient&&s.add(p.recipient.trim())});const i=s.size,l=r.breakdown_by_program.length;n.innerHTML=`
  ...
  `}function v(o){const a=document.getElementById("county-breakdown-list");let r=0;const n=[];o.forEach(i=>{const l=i.county_name.toUpperCase();l.includes("STATEWIDE")||l.includes("UNKNOWN")||l.includes("CENTRALIZED")?r+=i.total_loss:n.push(i)}),n.sort((i,l)=>l.total_loss-i.total_loss);const t=n.map(i=>`
    <tr>
  ...
  `}function T(o){const a=document.getElementById("cuts-table-body"),r=o.sort((n,t)=>n.amount-t.amount);a.innerHTML=r.map(n=>`
    <tr>
      <td style="white-space:nowrap; color: #6b7280; font-size: 0.85rem;">${b(n.date)}</td>
      <td style="font-weight: 500;">${n.agency}</td>
      <td>
        <div style="font-weight:600; color: #111;">${n.recipient||"Unknown"}</div>
        <div style="font-size:0.8rem; color:#6b7280">${n.county?.county_name||"Statewide"}, MD</div>
      </td>
  ...
  `).join("")}function $(o,a){const r=L.map("map").setView([39,-77.1],7);L.tileLayer("https://{s}.basemaps.cartocdn.com/dark_all/{z}/{x}/{y}{r}.png",{maxZoom:19,attribution:'&copy; <a href="https://www.openstreetmap.org/copyright">OpenStreetMap</a> contributors &copy; <a href="https://carto.com/attributions">CARTO</a>'}).addTo(r);const n={};a.forEach(e=>{e.county_name&&(n[e.county_name]=e.total_loss)});function t(e){return e>1e7?"#7f0000":e>5e6?"#b30000":e>2e6?"#d7301f":e>1e6?"#ef6548":e>5e5?"#fc8d59":e>1e5?"#fdbb84":e>10?"#fdd49e":"#374151"}function s(e){const d=(e.properties||{}).COUNTY,f=n[d]||0;return{fillColor:t(f),weight:1,opacity:1,color:"#1f2937",dashArray:"1",fillOpacity:.8}}const i=L.control();i.onAdd=function(e){return this._div=L.DomUtil.create("div","info"),this.update(),this._div},i.update=function(e){if(!e){this._div.innerHTML='<h4 style="margin:0; color:#374151;">County Impact</h4><span style="font-size:0.9em; color:#6b7280">Hover over a county</span>';return}const c=e.COUNTY||"Unknown",d=n[c]||0;this._div.innerHTML=`<h4 style="margin:0 0 5px 0; color:#111;">${c}</h4>`+(d?`<b style="font-size:1.1em; color:#BF0D3E;">${u.format(d)}</b> lost`:'<span style="color:#6b7280">No identified cuts</span>')},i.addTo(r);function l(e){var c=e.target;c.setStyle({weight:3,color:"#d1d5db",dashArray:"",fillOpacity:1}),c.bringToFront(),i.update(c.feature.properties)}function p(e){y.resetStyle(e.target),i.update()}const y=L.geoJson(o,{style:s,onEachFeature:function(e,c){c.on({mouseover:l,mouseout:p})}}).addTo(r),m=L.control({position:"bottomright"});m.onAdd=function(e){const c=L.DomUtil.create("div","info legend"),d=[0,1e5,5e5,1e6,2e6,5e6,1e7];c.innerHTML='<strong style="display:block; margin-bottom:5px; color:#374151;">Loss Amount</strong>';for(let f=0;f<d.length;f++)c.innerHTML+='<i style="background:'+t(d[f]+1)+'; width:18px; height:18px; float:left; margin-right:8px; opacity:0.8;"></i> <span style="color:#4b5563;">$'+(d[f]/1e6).toFixed(1)+"M"+(d[f+1]?"&ndash;$"+(d[f+1]/1e6).toFixed(1)+"M</span><br>":"+</span>");return c},m.addTo(r)}w();
